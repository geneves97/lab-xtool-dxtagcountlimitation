...
import br.jus.tre_pa.app.repository.vw.TipoPessoaListViewRepository;
...
@RestController
@AllArgsConstructor
@RequestMapping("/api/tipo-pessoa")
public class TipoPessoaRest extends StandardRest<TipoPessoa,Long> {

    private final TipoPessoaListViewRepository tipoPessoaListViewRepository;

    @Override
    public ResponseEntity<PageEx<?>> findAll(RSQLParam q, Pageable pageable, SummaryOptions summaryOptions) {
        return ResponseEntity.ok(
            tipoPessoaListViewRepository.findAll(
                q.getSpecification(),
                pageable,
                summaryOptions
            )
    ...
    public ResponseEntity<Resource> export(RSQLParam q, ExportOptions exportOptions) {
        ExportResult exportResult = tipoPessoaListViewRepository.export(q.getSpecification(), exportOptions);
