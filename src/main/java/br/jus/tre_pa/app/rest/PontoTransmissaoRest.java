...
import br.jus.tre_pa.app.repository.vw.PontoTransmissaoListViewRepository;
...
@RestController
@AllArgsConstructor
@RequestMapping("/api/ponto-transmissao")
public class PontoTransmissaoRest extends StandardRest<PontoTransmissao,Long> {

    private final PontoTransmissaoListViewRepository pontoTransmissaoListViewRepository;

    @Override
    public ResponseEntity<PageEx<?>> findAll(RSQLParam q, Pageable pageable, SummaryOptions summaryOptions) {
        return ResponseEntity.ok(
            pontoTransmissaoListViewRepository.findAll(
                q.getSpecification(),
                pageable,
                summaryOptions
            )
    ...
    public ResponseEntity<Resource> export(RSQLParam q, ExportOptions exportOptions) {
        ExportResult exportResult = pontoTransmissaoListViewRepository.export(q.getSpecification(), exportOptions);
